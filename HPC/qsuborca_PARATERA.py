...
if __name__ == '__main__':        
    ...
    total_mem = 256*1024*1024*1024*0.9 #余10%给系统防止占虚拟内存
...
#            input_filenames.pop(qos_option_index)
    ...
    for file_count,input_filename in enumerate(input_filenames):
        ...
        if re_ret or specified_resource_portion!=-1:
            ...
            job_memory_in_MB = int(total_mem*portion/1024/1024/portion_core*0.8)
            
            specified_boost_portion = float(specified_boost_portion)
            if specified_boost_portion>1:
                portion_mem = '%maxcore '+str(int(job_memory_in_MB*specified_boost_portion))
                portion_core="%pal nprocs "+str(int(portion_core/specified_boost_portion*specified_node))+" end"
            else:
        ...
        if mkl_file:
            command = " ".join(["$ORCA/orca_2mkl",filename_class(mkl_file).only_remove_append,'-gbw'])
            print("Calling", command)
            subprocess.run(command,shell=True,check=True)
            gbw_file = filename_class(mkl_file).replace_append_to('gbw')
            assert os.path.isfile(gbw_file),'GBW file conversion failed'
...
'''
        ...
        if not mkl_file:
            ret = ret.replace("cp [gbw_file] [gbw_new_name]","")
        else:
            ret=ret.replace('[gbw_file]',gbw_file)
            ret=ret.replace('[gbw_new_name]',os.path.join("$ORCA/temp/[NAME]".replace('[NAME]',job_name),base_name+".gbw"))
